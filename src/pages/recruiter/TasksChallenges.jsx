...
function TaskRow({ task, onAdvance }) {
  const meta = STAGE_META[task.status] || STAGE_META.assigned
  const idx = STAGES.indexOf(task.status)
  const next = STAGES[idx + 1]
  return (
    <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:T.cream2, border:`1px solid ${T.border}`, borderRadius:12, marginBottom:8 }}>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:13, fontWeight:600, color:T.ink }}>{task.title}</div>
        <div style={{ fontSize:11, color:T.ink4, marginTop:2 }}>{task.candidate_name || "—"}</div>
      </div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ background:T.cream, border:`1px solid ${T.border}`, borderRadius:16, padding:20, boxShadow:T.shadow }}>
        {loading ? (
          ...
          tasks.map((t) => <TaskRow key={t.id} task={t} onAdvance={advance} />)
        )}
      </div>
    </div>
  )
